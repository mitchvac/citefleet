...
- The console is PUBLIC in v1 (`VITE_AUTH_ENABLED=false`; `authMiddleware` throws when auth is off with a `DATABASE_URL`, so no server fn uses it), so `maskStoreSecrets` (secrets.ts, via `stripSecrets`) masks the GitHub token and every `site.webhook.secret` in every store copy a browser receives, including `resetState`. `webhookSecretFn` never returns an existing secret: it always mints a new one (`rotateWebhookSecret`) and returns it once. Anyone can therefore rotate a secret in v1 (as anyone can remove a property); enabling sign-in (`VITE_AUTH_ENABLED=true` + `authMiddleware` on the mutating server fns) is the real fix and is out of v1 scope.
