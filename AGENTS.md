...
| `deploy/` | Shared-VPS deploy: `DEPLOY-VPS.md`, `deploy-vps.sh` (re-execs from a private copy; mints the BotCentral service token and the operator token on first run; **regenerates `.env` from scratch every run**, so an env var added by hand on the box is wiped — add it here, reading from a `/root/citefleet-*` file, as `citefleet-gmail.smtp` does for password-reset mail. Passing no `DATABASE_URL` argument silently falls back to a LOCAL postgres container: always pass the live one), `run-detached.sh`, `nginx-citefleet.app.conf`, `.env.production.example`. Container binds `127.0.0.1:3021`. |
...
- Billing: CiteFleet **never charges anyone**, and there is no code that could. A `Site` carries no key,
  prefix, plan or balance, so there is nothing to bill against. Publishing goes to BotCentral
  `/internal/publish` with `BOTCENTRAL_SERVICE_TOKEN` — an internal endpoint, not the metered `/v1/*`
  consumer path — and it sits behind the `catalog` kill door, not `spend`. Catalog reads
  (`lookupListing`) send **no** credential at all, so they take BotCentral's free anonymous tier
  (30/min per IP). The only money surface is `/topup`, which credits a customer's own `bc_live_` key
  and is behind the `spend` door. So "I listed a site and was not charged" is the current design, not
  a broken meter: a listing is an operator action. Charging for one would need a billing identity on
  `Site` first — it does not exist yet.
