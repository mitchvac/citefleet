...
- Webhook: `site.webhook.secret` is shown to the operator on the campaign (this console is operator-only); `/api/hooks/github` verifies `X-Hub-Signature-256`, accepts push-to-branch and successful deployment_status, then `runWebhookListing` (proof with retries → publish) in the background.
