...
- BotCentral top-up (`/topup`): CiteFleet never verifies a chain payment. The customer opens an invoice
  ...
  A scan-to-pay QR appears on an open invoice ONLY when BotCentral returns `pay.address`; production returns `address: null`
  because no `BOTCENTRAL_PAY_*` env var is set on the catalog host, so today the page shows the pay-out-of-band note instead.
