...
- The console is PUBLIC in v1 (`VITE_AUTH_ENABLED=false`, no auth middleware on server fns), so `stripSecrets` masks both the GitHub token and every `site.webhook.secret` before the store reaches a browser; the webhook secret is returned once by `webhookSecretFn` (generate/rotate) and never again.
